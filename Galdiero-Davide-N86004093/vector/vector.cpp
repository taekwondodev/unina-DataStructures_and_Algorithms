...
Vector<Data>& Vector<Data>::operator=(const Vector<Data>& arr) {
    ...
    std::swap(*arr, *this);  //scambio arr e il mio vector attraverso i loro puntatori
...
//Sortable
template <typename Data>
SortableVector<Data>::SortableVector(const unsigned long int num){
    this->size = num;
    this->elements = new Data[this->size]{};
}

template <typename Data>
SortableVector<Data>::SortableVector(const TraversableContainer<Data>& con) : Vector<Data>(con.Size()) {
    unsigned long int index = 0;
...
template <typename Data>
SortableVector<Data>::SortableVector(MappableContainer<Data>&& con) : Vector<Data>(con.Size()) {
    unsigned long int index = 0;
...
SortableVector<Data>& SortableVector<Data>::operator=(const SortableVector<Data>& arr) {
    SortableVector<Data>* tmp_arr = new SortableVector<Data>(arr);  //ho bisogno di un puntatore che punta arr come this
