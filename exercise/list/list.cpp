...
bool List<Data>::Node::operator==(const Node& nod) const noexcept{
    if ((this->next == nullptr && nod.next == nullptr) || (this->next != nullptr && nod.next != nullptr)) {
...
List<Data>::List(const List& l){
    if (l.tail != nullptr){                  //se l ha almeno un elemento
        ...
        for(; current_node != nullptr; current_node = current_node->next){
            InsertAtBack(current_node->element);
        }
    }
}
...
bool List<Data>::operator==(const List<Data>& l) const noexcept{
    if (this->size == l.size) {                                             //se hanno la stessa size, scorro le liste e confronto i valori
        if (this->head == nullptr && l.head == nullptr){
        return true;
        }

        if ((this->head == nullptr && l.head != nullptr) || (this->head != nullptr && l.head == nullptr)){
        return false;
        }
        ...
        for (; current_node != nullptr; current_node = current_node->next, tmp_n = tmp_n->next) {
            if (current_node->element != tmp_n->element) {
                return false;
            }
        }
...
bool List<Data>::Remove(const Data& dat){
    ...
    while (*current_node != nullptr) {
        if ((*current_node)->element == dat){
            Node* tmp = *current_node;                     //se trovato, memorizzo

            *current_node = tmp->next;                     //aggiorno al successivo

            if (tmp == this->tail){                       //se era la coda, aggiorno il puntatore a nullptr
                this->tail = end;
            }

            tmp->next = nullptr;                          //per non cancellare tutta la lista
            delete tmp;
            this->size--;
        ...
        end = *current_node;                           //se non trovato continuo a scorrere
