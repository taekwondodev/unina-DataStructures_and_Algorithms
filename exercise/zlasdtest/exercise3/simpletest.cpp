...
void stestHashTableInt(lasd::HashTable<int> & ht, const lasd::LinearContainer<int> & con, uint & testnum, uint & testerr) {
  ...
  try {
    ...
    ht.Resize(2003);
    for (int i = 0; i < 127; i += 17) {
      InsertC(loctestnum, loctesterr, ht, con[i]); //devo incominciare a vedere da qui
    }
    ht.Resize(4001);
    Size(loctestnum, loctesterr, ht, true, 125);  //corretto
    ht.Resize(61);
    CountHT(loctestnum, loctesterr, ht, vec, 2);  //corretto
    Empty(loctestnum, loctesterr, ht, false);  //corretto
    ht.Clear();
    Empty(loctestnum, loctesterr, ht, true);  //corretto
    Size(loctestnum, loctesterr, ht, true, 0); //corretto
    InsertC(loctestnum, loctesterr, ht, con); //non inserisce tutti gli elementi o non li vede
    Empty(loctestnum, loctesterr, ht, false);   //corretto
    Size(loctestnum, loctesterr, ht, true, 125);  //mi da size 113
    CountHT(loctestnum, loctesterr, ht, vec, 2);  //corretto
  }
  catch (...) {
